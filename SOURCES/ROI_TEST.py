...
# ---------------------------------------------------------------------------
# Funcții locale pentru procesarea datelor (nu se vor muta în alt modul)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Funcții pentru analiza zonei de interes (ROI) a pachetului urmărit
# ---------------------------------------------------------------------------
import cv2
import math
import numpy as np

# Variabile globale pentru netezire
current_orientation = None  # valoarea raw detectată (opțional)
smoothed_orientation = None   # valoarea netezită
alpha = 0.2  # factorul de netezire (cu cât este mai mic, cu atât netezirea este mai puternică)

def analyze_tracked_box(image, tracked_pkg, margin=50):
    """
    Calculează zona de interes în jurul pachetului urmărit extinzând bounding box-ul cu margin (ex.: 50px)
    și analizează orientarea cutiei pe baza detectării marginilor.
    Returnează un dicționar cu coordonatele ROI, dimensiunile și unghiul mediu de orientare.
    """
    x, y = tracked_pkg["position"]
    if tracked_pkg.get("size") is not None and None not in tracked_pkg.get("size"):
        w, h = tracked_pkg["size"]
    else:
        w, h = 50, 50  # valori implicite dacă nu se cunosc dimensiuni
    ...
    # Preprocesare: aplicăm Gaussian blur pentru reducerea zgomotului
    gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
    gray = cv2.GaussianBlur(gray, (5, 5), 0)
    
    edges = cv2.Canny(gray, 50, 150, apertureSize=3)
    lines = cv2.HoughLinesP(edges, 1, math.pi/180, threshold=30, minLineLength=roi_width//4, maxLineGap=10)
    angles = []
    ...
    parameters = {
        ...
        "orientation_angle": avg_angle
    }
    return parameters

def print_tracked_box_parameters(image, tracked_pkg):
    params = analyze_tracked_box(image, tracked_pkg, margin=50)
    ...
    if params['orientation_angle'] is not None:
        print(f"Orientation Angle: {params['orientation_angle']:.2f}°")
    else:
        print("Orientation Angle: Not detected")
    print("===============================")
    cv2.rectangle(image, params['roi_top_left'], params['roi_bottom_right'], (255, 255, 0), 2)
...
# ---------------------------------------------------------------------------
# Importuri pentru detecția literelor și a cutiilor
# (Presupunem că modulele BOX_DETECT sunt disponibile în mediul de rulare)
# ---------------------------------------------------------------------------
...
def camera_loop():
    ...
    while True:
        ...
        # Detectare litere și cutii (presupunem că funcțiile detect_letters și detect_objects sunt definite)
        ...
        if tracked_package is not None:
            ...
            raw_orientation = print_tracked_box_parameters(image, tracked_package)
            # Actualizează valorile globale:
            if raw_orientation is not None:
                ...
                if smoothed_orientation is None:
                    ...
                    # Actualizare cu filtrare medie ponderată
...
def update_canvas():
    ...
    for (x, y) in corners:
        ...
        # Ajustare pentru coordonatele canvas (punctul de origine este în stânga sus)
        rotated_corners.append((center[0] + x_rot, center[1] - y_rot))
    
    # Desenăm pătratul rotit
    canvas.create_polygon(rotated_corners, outline="blue", fill="", width=2)
    # Desenăm o cruce în centru
    ...
    # Afișăm valoarea unghiului corectat
