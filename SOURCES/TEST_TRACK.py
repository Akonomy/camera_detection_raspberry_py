...
# Importăm funcția de tracking din modulul track_package din BOX_DETECT
from BOX_DETECT.track_package import determine_tracked_box
...
# ---------------------------------------------------------------------------
# Funcții locale de procesare și desenare
# ---------------------------------------------------------------------------
...
def camera_loop():
    ...
    while True:
        ...
        if session_data:
            # Apelăm funcția de tracking din track_package.py cu target fix: ("Sample", "A") și mandatory True
            tracked_info, flag1, flag2, access_level, blocking_boxes = determine_tracked_box(
                session_data,
                target_box=("Sample", "A"),
                mandatory=True,
                zone_center=zone_center
            )
            tracked_package = tracked_info
            # Pentru compatibilitate cu funcțiile de desen, asigurăm că avem cheia "letters"
            if "letter" in tracked_package and "letters" not in tracked_package:
                tracked_package["letters"] = [tracked_package["letter"]]
        ...
        if tracked_package is not None:
            ...
            # Folosim funcția din modulul de analiză pentru a calcula unghiul de înclinare
            inclination_angle = get_box_inclination_angle(original_image, tracked_package, margin=5, debug=False)
            
            # Actualizăm valorile de orientare
            ...
            # Calculăm distanța față de centrul zonei
            tracked_position = tracked_package.get("position", (0, 0))
            distance = math.sqrt((tracked_position[0] - zone_center[0])**2 + (tracked_position[1] - zone_center[1])**2)
            
            # Afișăm informații din sesiune
            print("=== Box Tracked Session Info ===")
            print(f"Color: {tracked_package.get('box_color')}")
            print(f"Letter: {tracked_package.get('letters')}")
            print(f"Position: {tracked_package.get('position')}")
            print(f"Distance from center: {distance:.2f} px")
            print(f"Inclination Angle: {inclination_angle}°")
            print(f"Access Level: {access_level}")
            print(f"Blocking Boxes: {blocking_boxes}")
            print("================================")
            
        mark_zone(image, zone_top_left, zone_bottom_right, label="Defined Zone")
        add_grid(image)
        
        # 6. Afișăm imaginea procesată
